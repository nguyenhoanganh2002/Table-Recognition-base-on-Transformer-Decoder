...
class PubtabnetParser(object):
    def __init__(self, jsonl_path, is_toy=True, split='val', is_pse_preLabel=False, chunks_nums=16):
        self.c_vocab_path = f'/kaggle/working/groundtruth_{split}/content_vocab'
        self.split = split
        self.raw_img_root = f'/kaggle/input/train-data3/pubtabnet/{split}/'
        self.save_root = f'/kaggle/working/groundtruth_{split}/'
    ...
    def parse_single_chunk(self, this_chunk, chunks_idx):
        ...
        c_vocab = set()
        max_seq_length = 0
        sum_length, n_seq = 0, 1
        max_cont_len = 0
        sum_cont_len, n_cont = 0, 1
#         progress_bar = tqdm(self.data_generator)
        for item in self.data_generator:
            ...
            #     continue
            l_seq = len(token_list)
            sum_length += l_seq
            n_seq += 1
            if l_seq > max_seq_length:
                max_seq_length = l_seq
            ...
            for idx, cell in enumerate(cells):
                if 'bbox' not in cell.keys():
                    ...
                    sum_cont_len += l_cont
                    n_cont += 1
                    if l_cont > max_cont_len:
                        max_cont_len = l_cont
                    c_vocab.update(content)
        ...
        anott = "\n".join(c_vocab)
        anott += (f"\n{max_seq_length} {sum_length/n_seq}")
        anott += (f"\n{max_cont_len} {sum_cont_len/n_cont}")
        open(self.c_vocab_path + f"/chunk_{chunks_idx}.txt", "w").write(anott)
    ...
    def merge_chunks(self):
        c_vocab = set()
        for dir in os.listdir(self.c_vocab_path):
            c_i = open(self.c_vocab_path + "/" + dir, "r").read().split("\n")
            c_vocab.update(c_i[:-2])
            
        c_vocab = sorted(list(c_vocab))
        open(self.alphabet_path_2, "w").write("\n".join(c_vocab))
...
if __name__ == '__main__':
    ...
    jsonl_path = r'/kaggle/input/train-data3/pubtabnet/PubTabNet_2.0.0.jsonl'

    parser = PubtabnetParser(jsonl_path, is_toy=False, split=split, is_pse_preLabel=True, chunks_nums=nproc)
